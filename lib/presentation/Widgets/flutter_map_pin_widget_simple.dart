...
class FlutterMapPinWidgetSimple extends StatefulWidget {
  final List<Pin> pins;
  ...
  const FlutterMapPinWidgetSimple({
    Key? key,
    required this.pins,
  ...
  @override
  State<FlutterMapPinWidgetSimple> createState() =>
      FlutterMapPinWidgetSimpleState();
}

class FlutterMapPinWidgetSimpleState extends State<FlutterMapPinWidgetSimple> {
  ...
  @override
  void didUpdateWidget(FlutterMapPinWidgetSimple oldWidget) {
  ...
  Future<Uint8List> _createSimpleCustomMarker(Pin pin) async {
    ...
    final moodPaint = Paint()
      ..color = Colors.white // Changed to yellow for better visibility
      ..style = PaintingStyle.fill;

    // Position mood icon half inside, half outside the main circle (bottom-right)
    final moodCenter = Offset(
      size.width / 2 + 45, // Increased offset to move mood icon more outside
      size.height / 2 + 45, // Increased offset to move mood icon more outside
    );

    canvas.drawCircle(
      moodCenter,
      18, // Increased mood icon radius for better visibility
      moodPaint,
    );
    ...
    try {
      ...
      if (pin.moodIconUrl.isNotEmpty && pin.moodIconUrl.startsWith('http')) {
        ...
        if (moodResponse.statusCode == 200) {
          ...
          final moodRect = Rect.fromCircle(center: moodCenter, radius: 14);
      ...
      canvas.drawCircle(
        moodCenter,
        14, // Increased placeholder mood radius
      ...
      // Draw eyes
      canvas.drawCircle(
        Offset(moodCenter.dx - 6, moodCenter.dy - 5),
        3.5, // Increased eye size
        smileyPaint,
      );
      canvas.drawCircle(
        Offset(moodCenter.dx + 6, moodCenter.dy - 5),
    ...
    final dotCenter = Offset(
      ...
      size.height - 7, // Increased offset for better positioning
    );

    // Add shadow to green dot
    final dotShadowPaint = Paint()
      ..color = Colors.black.withOpacity(0.45) // Increased shadow opacity
      ..maskFilter =
          const MaskFilter.blur(BlurStyle.normal, 2.5); // Increased shadow blur

    canvas.drawCircle(
      Offset(dotCenter.dx + 1, dotCenter.dy + 1),
      4, // Increased green dot shadow radius
      dotShadowPaint,
    );

    canvas.drawCircle(
      dotCenter,
      5, // Increased green dot radius for better visibility
      dotPaint,
    );
